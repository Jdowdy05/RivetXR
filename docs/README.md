# RIVET XR documentation

- [Project quick start](../README.md)
- [Building and prerequisites](BUILDING.md)
- [Device validation and remaining checks](VALIDATION.md)
- [Remote scene streaming, recording and controls](REMOTE_SCENE.md)
- [Showcase media and reproduction](MEDIA.md)
- [Third-party notices](../THIRD_PARTY_NOTICES.md)
- [Contributing](../CONTRIBUTING.md)

This source release includes the native Quest application, the host remote
pipeline and synthetic tests. See the quick start's current limits before
assuming a feature has been qualified with a headset, camera or physical robot.
